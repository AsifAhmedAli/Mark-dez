...
import Buttons from "./expense_buttons";
import All_expense_heads1 from "./all_expense_head";
...
const Expense = () => {
  ...
  const [expense_head_data, setexpense_head_data] = useState(false);
  const All_expenses = async (limit1) => {
    setloading(true);
    ...
    var urla =
      "https://mexil.it/chroma_api/all_expenses_with_limit.php?pagi=" + limit;
    var config = {
      method: "get",
      url: urla,
      headers: {
  ...
  const reset_limit = () => {
    ...
    const incri = 20;
    // alert("asdfasdf");
    setexpense_head_data(false);
    All_expenses(incri);
  };
  const view_expense_heads = async () => {
    // alert("asdfasdf");
    // window.location.reload();
    setexpense_head_data(true);
  };
  useEffect(() => {
    const incri = 20;
    All_expenses(incri);
  ...
  var data1 = [
    {
      view_expense_heads: view_expense_heads,
